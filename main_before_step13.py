  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .banner{
      margin-bottom:14px;
      padding:12px 14px;
      border:1px solid #6a4c1a;
      border-radius:14px;
      background:linear-gradient(180deg,#23170e,#2d1d12);
      color:#ffd79b;
    }
  ...
  <div class="wrap">
    <div class="topbar">
      <div>
        ...
        <div class="sub">Read-only intelligence dashboard powered by /terminal-data, /agent-status, /scheduler-status, and /source-health</div>
      </div>
      <div class="btns">
    ...
    <div class="footer">Breadth stays limited until NEWSAPI_KEY and GUARDIAN_API_KEY are configured. Market prices stay empty until ALPHAVANTAGE_KEY is configured.</div>
  </div>
...
function renderHealthPanels(){
  ...
  let html = sources.map(s =>
    '<div class="mini">' +
      '<div class="row"><div><strong>' + esc(s.name || '') + '</strong></div><div class="' + ((s.ready ? 'status-good' : 'status-warn')) + '">' + (s.ready ? 'ready' : 'limited') + '</div></div>' +
      '<div class="small" style="margin-top:6px">Enabled: ' + esc(String(!!s.enabled)) + '</div>' +
      '<div class="small" style="margin-top:4px">' + esc(s.note || '') + '</div>' +
    '</div>'
  ).join('');

  if (recentErrors.length){
    html += '<div class="mini"><div><strong>Recent source errors</strong></div>' +
      recentErrors.slice(0,4).map(x =>
        '<div class="small status-warn" style="margin-top:6px">' + esc((x.started_at || '') + ' · ' + (x.error_text || '')) + '</div>'
      ).join('') +
      '</div>';
  }
...
async function reloadAll(){
  try{
    ...
    const [terminalData, agentStatus, schedulerStatus, sourceHealth] = await Promise.all([
