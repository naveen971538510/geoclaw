  ...
  <div class="wrap">
    <div class="topbar">
      <div>
        ...
        <div class="sub">Read-only intelligence dashboard powered by /terminal-data</div>
      </div>
      <div class="btns">
...
async function reloadNow(){
  try{
    document.getElementById('statusBox').textContent = 'Loading /terminal-data ...';
    const res = await fetch('/terminal-data', {cache:'no-store'});
    if (!res.ok) throw new Error('HTTP ' + res.status);
    const data = await res.json();
    if (data.status === 'error') throw new Error(data.error || 'terminal-data error');
    ...
    document.getElementById('statusBox').textContent =
      ...
      ' | Cards: ' + String((data.cards || []).length) +
      ' | Alerts: ' + String(((data.top_alerts || []).length));
  }catch(err){
...
document.getElementById('refreshBtn').addEventListener('click', reloadNow);
