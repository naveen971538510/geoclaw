...
from config import DB_PATH, ENABLE_GDELT, ENABLE_GUARDIAN, ENABLE_NEWSAPI, ENABLE_RSS, DEFAULT_WATCHLIST
...
def _store_alert(cur, article_id: int, ranking: Dict, enrichment: Dict) -> int:
    if ranking.get("priority") not in ("high", "urgent"):
        return 0
    reason = ", ".join(enrichment.get("alert_tags", []) or enrichment.get("watchlist_hits", []) or ["impact"])
    cur.execute(
        """
        INSERT INTO alert_events (article_id, priority, reason, created_at, is_read)
        VALUES (?, ?, ?, ?, 0)
        """,
        (article_id, ranking.get("priority", "watch"), reason, utc_now_iso()),
    )
    return 1
...
def run_ingestion_cycle(query: str = None, max_records_per_source: int = 20, watchlist: List[str] = None) -> Dict:
    ...
    for name, source in _get_sources():
        try:
