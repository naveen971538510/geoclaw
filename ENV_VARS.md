...
| `GEOCLAW_TRUSTED_HOSTS` | Optional comma-separated Host-header allow-list applied to both `main.py` and `dashboard_api.py` via Starlette's `TrustedHostMiddleware`. When set, any request whose `Host:` header is not in the list gets 400 "Invalid host header". Defense against HTTP Host header injection (e.g. attacker forging `Host: evil.com` to poison cached absolute URLs, password-reset links, or webhook-reply targets). Wildcards work the way Starlette accepts them — `*.example.com` matches `a.example.com` but NOT the apex `example.com`, so include both if you need both. `localhost`, `127.0.0.1`, and `testserver` are always implicitly added so healthchecks and unit tests keep working. Off by default so deploys behind proxies that strip / rewrite the Host header don't break. Example: `GEOCLAW_TRUSTED_HOSTS=app.example.com,*.example.com`. |
