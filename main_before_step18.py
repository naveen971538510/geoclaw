...
<head>
  ...
  <style>
    :root{
      --bg:#07090d; --panel:#10151c; --panel2:#161d27; --line:#232c39; --text:#e8edf5;
      --muted:#90a0b7; --green:#18c37b; --red:#ff5f56; --yellow:#ffd166; --blue:#56a8ff;
      --cyan:#4de2ff; --purple:#a78bfa;
    }
    *{box-sizing:border-box}
    body{margin:0;background:radial-gradient(circle at top right, rgba(86,168,255,.08), transparent 24%),radial-gradient(circle at top left, rgba(77,226,255,.05), transparent 24%),var(--bg);color:var(--text);font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace;}
    ...
    .banner{margin-bottom:14px;padding:12px 14px;border:1px solid #6a4c1a;border-radius:14px;background:linear-gradient(180deg,#23170e,#2d1d12);color:#ffd79b}
    .btns{display:flex;gap:10px;flex-wrap:wrap}
    .btn,.pill{border:1px solid var(--line);background:linear-gradient(180deg,var(--panel),var(--panel2));color:var(--text);border-radius:12px;padding:10px 14px;text-decoration:none;cursor:pointer;font-weight:800}
    .btn.primary{background:linear-gradient(180deg,#102542,#153157);border-color:#245a96;box-shadow:0 0 0 1px rgba(86,168,255,.15) inset}
    .btn.warn{background:linear-gradient(180deg,#2e1a10,#4b2a18);border-color:#8b4c2d;box-shadow:0 0 0 1px rgba(255,209,102,.12) inset}
    .hero{display:grid;grid-template-columns:repeat(6,minmax(120px,1fr));gap:12px;margin-bottom:14px}
    .stat{background:linear-gradient(180deg,var(--panel),var(--panel2));border:1px solid var(--line);border-radius:16px;padding:14px;min-height:88px}
    .stat .k{font-size:11px;color:var(--muted);letter-spacing:.06em}.stat .v{font-size:26px;font-weight:900;margin-top:8px}
    .marketstrip{display:grid;grid-template-columns:repeat(6,minmax(120px,1fr));gap:12px;margin-bottom:14px}
    .ticker{background:linear-gradient(180deg,var(--panel),var(--panel2));border:1px solid var(--line);border-radius:14px;padding:12px}
    .ticker .n{font-size:11px;color:var(--muted)} .ticker .p{font-size:18px;font-weight:900;margin-top:4px}
    .ticker .c.up{color:var(--green)} .ticker .c.down{color:var(--red)} .ticker .c.flat{color:var(--yellow)}
    .controls{display:grid;grid-template-columns:2fr 1fr 1fr 1fr auto auto auto;gap:10px;margin-bottom:14px}
    .input,.select{width:100%;padding:12px 13px;border:1px solid var(--line);border-radius:12px;background:linear-gradient(180deg,var(--panel),var(--panel2));color:var(--text);outline:none}
    .toggle{display:flex;align-items:center;gap:8px;border:1px solid var(--line);border-radius:12px;background:linear-gradient(180deg,var(--panel),var(--panel2));color:var(--muted);padding:12px 13px;white-space:nowrap}
    .layout{display:grid;grid-template-columns:minmax(0,1fr) 390px;gap:14px}
    .feed{display:grid;gap:12px}
    .card,.panel{background:linear-gradient(180deg,var(--panel),var(--panel2));border:1px solid var(--line);border-radius:16px;padding:14px;box-shadow:0 10px 30px rgba(0,0,0,.18)}
    .card.bullish{border-left:4px solid var(--green)} .card.bearish{border-left:4px solid var(--red)} .card.neutral{border-left:4px solid var(--yellow)}
    .row{display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap}
    .leftline,.rightline{display:flex;align-items:center;gap:8px;flex-wrap:wrap}
    .signal{padding:5px 9px;border-radius:999px;font-size:11px;font-weight:900;border:1px solid transparent}
    ...
    .badge{padding:4px 8px;border-radius:999px;font-size:11px;font-weight:800;border:1px solid var(--line);color:var(--muted)}
    .badge.asset{color:var(--blue)} .badge.alert{color:var(--red)} .badge.watch{color:var(--purple)}
    .score{padding:6px 9px;border-radius:10px;border:1px solid var(--line);background:rgba(255,255,255,.02);font-size:12px;font-weight:900;color:var(--cyan)}
    .headline{display:block;color:var(--text);text-decoration:none;font-size:18px;font-weight:900;line-height:1.42;margin:12px 0 10px 0}
    .headline:hover{text-decoration:underline}
    .summary,.meta,.small,.footer,.time,.source{color:var(--muted);font-size:12px}
    .thesis{margin-top:10px;padding:12px;border:1px solid var(--line);border-radius:12px;background:rgba(255,255,255,.02)}
    .cases{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:10px}
    .case{padding:10px;border:1px solid var(--line);border-radius:12px;background:rgba(255,255,255,.02)}
    .actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:12px}
    .linkbtn{display:inline-flex;align-items:center;gap:6px;text-decoration:none;color:var(--text);border:1px solid var(--line);padding:8px 10px;border-radius:10px;background:rgba(255,255,255,.02)}
    .panel h3{margin:0 0 12px 0;font-size:12px;color:var(--muted);letter-spacing:.08em}
    .mini{padding:10px;border:1px solid var(--line);border-radius:12px;background:rgba(255,255,255,.02);margin-bottom:10px}
    .mini a{color:var(--text);text-decoration:none;font-size:13px;font-weight:800;line-height:1.35}.mini a:hover{text-decoration:underline}
    .barwrap{height:8px;border-radius:999px;background:rgba(255,255,255,.05);overflow:hidden;margin-top:8px}
    .barfill{height:100%;background:linear-gradient(90deg,var(--cyan),var(--blue));border-radius:999px}
    .sticky{position:sticky;top:12px}
    .empty{padding:28px;border:1px dashed var(--line);border-radius:16px;color:var(--muted);text-align:center;background:linear-gradient(180deg,var(--panel),var(--panel2))}
    .status-good{color:var(--green)} .status-warn{color:var(--yellow)} .status-bad{color:var(--red)}
    @media (max-width:1100px){.layout{grid-template-columns:1fr}.hero{grid-template-columns:repeat(3,minmax(120px,1fr))}.marketstrip{grid-template-columns:repeat(3,minmax(120px,1fr))}.controls{grid-template-columns:1fr 1fr}.cases{grid-template-columns:1fr}}
    @media (max-width:760px){.hero{grid-template-columns:repeat(2,minmax(120px,1fr))}.marketstrip{grid-template-columns:repeat(2,minmax(120px,1fr))}.controls{grid-template-columns:1fr}}
  </style>
</head>
<body>
  <div class='wrap'>
    <div class='topbar'>
      <div>
        ...
        <div class='sub'>Read-only intelligence dashboard powered by /terminal-data, /agent-status, /scheduler-status, /source-health, and /what-changed</div>
      </div>
      <div class='btns'>
        ...
        <a class='btn' href='/source-health' target='_blank' rel='noopener noreferrer'>SOURCE HEALTH</a>
      </div>
    </div>

    <div id='warningBanner' class='banner' style='display:none'></div>
    ...
    <section class='controls'>
      ...
      <select id='asset' class='select'><option value=''>All assets</option></select>
      ...
      <label class='toggle'><input type='checkbox' id='trustedOnly'> trusted only</label>
      <button class='btn' id='resetFiltersBtn'>RESET FILTERS</button>
    </section>

    <div id='resultsBar' class='banner' style='display:block;margin-top:-2px;margin-bottom:14px;background:linear-gradient(180deg,#0e1b26,#102334);border-color:#1e3a5c;color:#b8ddff'>
      Results: loading...
    </div>
    ...
    <section class='layout'>
      ...
      <div class='sticky'>
        <div class='panel'>
          <h3>WHAT CHANGED</h3>
          <div id='whatChangedBox' class='small'>Loading…</div>
        </div>

        <div class='panel' style='margin-top:12px'>
          <h3>KEY STATUS</h3>
          <div id='keyStatusBox' class='small'>Loading…</div>
        </div>

        <div class='panel' style='margin-top:12px'>
          <h3>SOURCE HEALTH</h3>
          <div id='sourceHealthBox' class='small'>Loading…</div>
        </div>
    ...
    <div class='footer'>Breadth stays limited until NEWSAPI_KEY and GUARDIAN_API_KEY are configured. Market prices stay partly limited until ALPHAVANTAGE_KEY responses fully populate.</div>
  </div>
...
function currentCards(){
  ...
  let cards = state.cards.filter(x => {
    const search = ((x.headline || '') + ' ' + (x.source || '') + ' ' + (x.summary || '') + ' ' + (x.thesis || '') + ' ' + (x.asset_tags || []).join(' ') + ' ' + (x.alert_tags || []).join(' ') + ' ' + (x.watchlist_hits || []).join(' ')).toLowerCase();
...
function renderBanner(){
  ...
  box.style.display = '';
  box.innerHTML = 'Missing keys: <strong>' + esc(missing.join(', ')) + '</strong>. Without them, wide news coverage and market snapshot remain limited.';
}
function renderMarket(){
  const box = document.getElementById('marketstrip');
  const items = ((state.payload || {}).market_snapshot || []).filter(x => x && x.price != null);
  const keys = ((state.sourceHealth || {}).keys || {});
  if (!items.length){
    box.innerHTML = '<div class="ticker"><div class="n">MARKET SNAPSHOT</div><div class="p">No data</div><div class="small">' + (keys.alphavantage_configured ? 'Waiting for snapshot' : 'Set ALPHAVANTAGE_KEY') + '</div></div>';
    return;
  }
  box.innerHTML = items.map(x => {
    ...
    if (pct > 0) cls = 'up';
    if (pct < 0) cls = 'down';
    return '<div class="ticker"><div class="n">' + esc(x.label || x.symbol) + '</div><div class="p">' + esc(x.price == null ? 'n/a' : String(x.price)) + '</div><div class="c ' + cls + '">' + esc(x.change_pct == null ? 'n/a' : String(x.change_pct) + '%') + '</div></div>';
  }).join('');
}
function renderWhatChanged(){
  const box = document.getElementById('whatChangedBox');
  ...
  let html = '';
  html += '<div>New articles (30m): <strong>' + esc(String(summary.new_articles || 0)) + '</strong></div>';
  html += '<div style="margin-top:6px">New alerts (30m): <strong>' + esc(String(summary.new_alerts || 0)) + '</strong></div>';
  html += '<div style="margin-top:6px">Latest run: <strong>' + esc(summary.latest_run_status || 'n/a') + '</strong></div>';

  if (Object.keys(delta).length){
    html += '<div class="mini" style="margin-top:10px">';
    html += '<div class="small">Run delta</div>';
    html += '<div style="margin-top:6px">Fetched Δ: ' + esc(String(delta.items_fetched_delta || 0)) + '</div>';
    html += '<div style="margin-top:4px">Kept Δ: ' + esc(String(delta.items_kept_delta || 0)) + '</div>';
    html += '<div style="margin-top:4px">Alerts Δ: ' + esc(String(delta.alerts_created_delta || 0)) + '</div>';
    html += '</div>';
  }

  if (recentArticles.length){
    html += '<div class="mini" style="margin-top:10px"><div><strong>Recent headlines</strong></div>';
    html += recentArticles.slice(0,4).map(x =>
      '<div class="small" style="margin-top:8px">[' + esc(x.source || '') + '] ' + esc(x.headline || '') + '</div>'
    ).join('');
    html += '</div>';
  }

  if (recentAlerts.length){
    html += '<div class="mini" style="margin-top:10px"><div><strong>Recent alerts</strong></div>';
    html += recentAlerts.slice(0,3).map(x =>
      '<div class="small" style="margin-top:8px">' + esc((x.priority || '').toUpperCase()) + ' · ' + esc(x.headline || '') + '</div>'
    ).join('');
    html += '</div>';
  }

  box.innerHTML = html || '<div class="small">No changes yet.</div>';
}
function renderHealthPanels(){
  const keyBox = document.getElementById('keyStatusBox');
  const sourceBox = document.getElementById('sourceHealthBox');
  const health = state.sourceHealth || {};
  ...
  const keyRows = [
    ['NEWSAPI_KEY', !!keys.newsapi_configured],
    ['GUARDIAN_API_KEY', !!keys.guardian_configured],
    ['ALPHAVANTAGE_KEY', !!keys.alphavantage_configured],
  ];

  keyBox.innerHTML = keyRows.map(([k, ok]) =>
    '<div class="' + (ok ? 'status-good' : 'status-warn') + '">' + esc(k) + ': ' + (ok ? 'configured' : 'missing') + '</div>'
  ).join('');

  let html = sources.map(s =>
    '<div class="mini"><div class="row"><div><strong>' + esc(s.name || '') + '</strong></div><div class="' + (s.ready ? 'status-good' : 'status-warn') + '">' + (s.ready ? 'ready' : 'limited') + '</div></div><div class="small" style="margin-top:6px">Enabled: ' + esc(String(!!s.enabled)) + '</div><div class="small" style="margin-top:4px">' + esc(s.note || '') + '</div></div>'
  ).join('');

  if (recentErrors.length){
    html += '<div class="mini"><div><strong>Recent source errors</strong></div>' +
      recentErrors.slice(0,4).map(x => '<div class="small status-warn" style="margin-top:6px">' + esc((x.started_at || '') + ' · ' + (x.error_text || '')) + '</div>').join('') +
      '</div>';
  }

  sourceBox.innerHTML = html || '<div class="small">No source health data.</div>';
}
function renderAgentPanels(){
  ...
  if (!jobs.length){
    ...
    schedulerBox.innerHTML =
      ...
      jobs.map(j => '<div class="mini" style="margin-top:8px"><div><strong>' + esc(j.id || '') + '</strong></div><div class="small" style="margin-top:6px">Next: ' + esc(j.next_run_time || 'n/a') + '</div><div class="small" style="margin-top:4px">Trigger: ' + esc(j.trigger || 'n/a') + '</div></div>').join('');
  }
  ...
  if (!runs.length){
    ...
    runsBox.innerHTML = runs.slice(0, 8).map(r => {
      ...
      return '<div class="mini"><div class="row"><div><strong>' + esc(r.run_type || 'run') + '</strong></div><div class="' + cls + '">' + esc(r.status || 'unknown') + '</div></div><div class="small" style="margin-top:8px">Fetched: ' + esc(String(r.items_fetched || 0)) + ' · Kept: ' + esc(String(r.items_kept || 0)) + ' · Alerts: ' + esc(String(r.alerts_created || 0)) + '</div><div class="small" style="margin-top:6px">Started: ' + esc(r.started_at || 'n/a') + '</div><div class="small" style="margin-top:4px">Finished: ' + esc(r.finished_at || 'n/a') + '</div>' + (r.error_text ? '<div class="small status-warn" style="margin-top:6px">Error: ' + esc(r.error_text) + '</div>' : '') + '</div>';
    }).join('');
  }
}
...
function renderCards(){
  ...
  const active = [];
  if (document.getElementById('signal').value) active.push('signal=' + document.getElementById('signal').value);
  if (document.getElementById('asset').value) active.push('asset=' + document.getElementById('asset').value);
  if (document.getElementById('source').value) active.push('source=' + document.getElementById('source').value);
  if (document.getElementById('alertsOnly').checked) active.push('alerts-only');
  if (document.getElementById('trustedOnly').checked) active.push('trusted-only');
  if (document.getElementById('q').value.trim()) active.push('search');

  resultsBar.innerHTML = 'Results: <strong>' + String(cards.length) + '</strong> / ' + String(state.cards.length) + (active.length ? ' · Active filters: ' + active.join(', ') : ' · No active filters');
  ...
  if (!cards.length){
    ...
    feed.innerHTML = cards.map(x => {
      ...
      return '<article class="card ' + sig + '"><div class="row"><div class="leftline"><span class="signal ' + sig + '">' + esc(x.signal || 'Neutral') + '</span><span class="source">' + esc(x.source || 'Unknown') + '</span>' + assets + alerts + watch + '</div><div class="rightline"><span class="score">IMPACT ' + esc(String(x.impact_score || 0)) + '</span><span class="time">' + esc(relTime(x.published_at || '')) + '</span></div></div><a class="headline" href="' + esc(x.url || '#') + '" target="_blank" rel="noopener noreferrer">' + esc(x.headline || '') + '</a><div class="summary">' + esc(x.summary || '') + '</div><div class="thesis"><div><strong>Thesis:</strong> ' + esc(x.thesis || 'No thesis yet') + '</div><div style="margin-top:8px"><strong>What to watch:</strong> ' + esc(x.what_to_watch || 'n/a') + '</div></div><div class="cases"><div class="case"><strong>Bull case</strong><div class="small" style="margin-top:6px">' + esc(x.bull_case || 'n/a') + '</div></div><div class="case"><strong>Bear case</strong><div class="small" style="margin-top:6px">' + esc(x.bear_case || 'n/a') + '</div></div></div><div class="actions"><a class="linkbtn" href="' + esc(x.url || '#') + '" target="_blank" rel="noopener noreferrer">OPEN ARTICLE</a><a class="linkbtn" href="/terminal-data" target="_blank" rel="noopener noreferrer">OPEN JSON</a></div><div class="meta" style="margin-top:10px">Published: ' + esc(x.published_at || 'n/a') + ' · Confidence: ' + esc(String(x.confidence || 0)) + '</div></article>';
    }).join('');
  }
...
async function reloadAll(){
  try{
    ...
    const [terminalData, agentStatus, schedulerStatus, sourceHealth, whatChanged] = await Promise.all([
      fetchJson('/terminal-data'),
      fetchJson('/agent-status'),
      fetchJson('/scheduler-status'),
      fetchJson('/source-health'),
      fetchJson('/what-changed'),
    ]);
...
['q','signal','asset','source','sort','alertsOnly','trustedOnly'].forEach(id => {
...
try:
    ...
    print("WARN: provider self-test boot failed:", exc)
